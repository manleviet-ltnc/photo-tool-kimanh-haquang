...
namespace MyAlbumEditor
{
    public partial class AlbumCalendar : UserControl
    {
        ...
        private void UpdateCalendar()
        {
            ...
            monDates.MinDate = minDate;
            monDates.MaxDate = maxDate;
            monDates.TodayDate = minDate;
            monDates.SelectionStart = minDate;
            monDates.UpdateBoldedDates();
        }

        private void monDates_MouseDown(object sender, MouseEventArgs e)
        {
            // Ignore click if no album selected
            if (Manager == null)
                return;

            MonthCalendar.HitTestInfo info = monDates.HitTest(e.X, e.Y);
            ...
            if (item != null && item.Tag is int)
            {
                ...
                using (PhotoEditDialog dlg = new PhotoEditDialog(Manager))
                {
                    ...
                    if (dlg.ShowDialog() == DialogResult.OK && oldDate != Manager.Current.DateTaken)
                        UpdateCalendar();   // DateTaken was modified
                }
            }
        }
    }
